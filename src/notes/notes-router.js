    ...
    for (const [key, value] of Object.entries(newNote)) {
      if (!value) {
        return res.status(400).json({
          error: {
            message: `New note must include name, folder_id, and content.  Missing ${key} in request body`
          }
        });
      }
    }
  ...
  .patch(jsonParser, (req, res, next) => {
    ...
    if (valuesToUpdate === 0) {
      return res.status(400).json({
        error: {
          message:
            "To update note please include name, date_modified, folder_id, or content."
        }
      });
    }
